...
import NoAdminAccess from "./HOC/NoAdminAccess";
...
function Messenger() {
  ...
  const getChatRoom = ({ item }) => {
    chatListUser.find((obj) => {
      if (obj._id === item._id) {
        ...
        userInfoLogged.name !== item.guestUserB.name
          ? setName(item.guestUserB.name + " " + item.guestUserB.lastName)
          : setName(item.guestUserA.name + " " + item.guestUserA.lastName);
      }
  ...
  return (
    <>
      {chatListUser.length === 0 && <Loading />}
      <audio ref={audioPlayer} src="/resources/notification_sound.mp3" />
      <div className="card">
                  ...
                  <div>
                    <div className="text-center">
                      {chatListUser.length === 0 ? (
                        <h1>You don not have a manager assigned</h1>
                      ) : (
...
export default NoAdminAccess(Messenger);
